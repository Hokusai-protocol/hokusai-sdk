...
export async function runPrivacyCli(
  argv: string[],
  ...
  if (parsed.subcommand === 'list') {
    ...
    if (!result.ok) {
      return fail(
        ...
        PRIVACY_CLI_EXIT_CODES.UNKNOWN_ERROR,
        parsed.json,
      );
    }
  ...
  if (parsed.subcommand === 'preview') {
    ...
    const result = await previewStoredDecision(
      {
  ...
  if (parsed.subcommand === 'clear') {
    ...
    const result = await clearPrivacyState(
      { scope: parsed.scope },
      {
        ...(parsed.configPath ? { configPath: parsed.configPath } : {}),
        env,
    ...
    return ok(
      `Cleared ${parsed.scope} privacy state.\n`,
      parsed.json,
      { subcommand: 'clear', result: result.value },
    );
  }
  ...
  if (parsed.subcommand === 'reporting') {
    ...
    if (parsed.action === 'status') {
      const result = await getReportingStatus({
        ...(parsed.configPath ? { configPath: parsed.configPath } : {}),
        env,
      });
      if (!result.ok) {
    ...
    const result = await setReportingEnabled(
      {
