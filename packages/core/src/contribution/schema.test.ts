import { describe, it, expect } from 'vitest';
import {
  TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION,
  TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION_V2,
  ContributionValidationError,
  HARNESS_OUTCOME_ROW_FIELDS,
  HARNESS_OUTCOME_ROW_SCHEMA_VERSION,
  validateContributionRow,
  isSubmitDataContributionRow,
} from './schema.js';
import type { HokusaiTaskDescriptor } from './descriptor-types.js';

function expectContributionError(
  fn: () => unknown,
  code: 'schema_validation_failed' | 'forbidden_field',
): void {
  let caught: unknown;
  try {
    fn();
  } catch (error) {
    caught = error;
  }
  expect(caught).toBeInstanceOf(ContributionValidationError);
  expect((caught as ContributionValidationError).code).toBe(code);
}

function makeTaskDescriptor(): HokusaiTaskDescriptor {
  return {
    task_type: 'feature',
    language: 'typescript',
    domain: 'frontend',
    complexity: 5,
    repo_size_bucket: 'medium',
    files_touched_bucket: '2_5',
    description_length_bucket: 'medium',
    is_greenfield: false,
    is_migration: false,
    requires_tests: true,
    cross_service: false,
    ui_heavy: true,
    risk_level: 'medium',
  };
}

function makeV2Row(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    schema_version: TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION_V2,
    task_descriptor: makeTaskDescriptor(),
    allowed_models: ['planner-a', 'qwen2.5-coder-32b', 'reviewer-a'],
    selected_models: {
      planner: 'planner-a',
      coder: 'qwen2.5-coder-32b',
      reviewer: 'reviewer-a',
    },
    available_models: {
      planner_models: ['planner-a'],
      coder_models: ['qwen2.5-coder-32b', 'deepseek-coder-v2'],
      reviewer_models: ['reviewer-a'],
    },
    budget_usd: 10,
    actual_cost_usd: 2.2,
    wall_clock_seconds: 120,
    success_under_budget: true,
    completion_result: 'success',
    outcome_labels: {
      budget_label: 'under_budget',
      cost_label: 'medium',
      time_label: 'fast',
      success_label: 'success',
    },
    candidate_pool: {
      scenario_id: 'challenger-qwen-vs-baseline',
      scenario_kind: 'challenger',
      pool_size: 2,
      baseline_model: 'gpt-5.4',
    },
    sparse_cell: {
      cell_id: 'frontend-medium-2_5',
      descriptor_signature: 'frontend|medium|2_5',
      observed_count: 3,
      is_sparse: true,
    },
    scorer_ref: 'technical_task_router.benchmark_score/v2',
    observed_at: '2026-05-30T12:00:00.000Z',
    task_id: 'redacted-task',
    harness: 'wavemill',
    ...overrides,
  };
}

describe('hokusai-contribution-schema', () => {
  it('validates a minimal submit-data row', () => {
    const row = validateContributionRow({ success_under_budget: true });
    expect(row).toEqual({ success_under_budget: true });
  });

  it('validates a full submit-data row', () => {
    const row = validateContributionRow({
      success_under_budget: false,
      inputs: { route_family: 'balanced', retries: 2, dry_run: false },
      actual_cost_usd: 1.23,
      wall_clock_seconds: 45,
      task_id: 'redacted-abc',
      harness: 'wavemill',
    });
    expect(row.actual_cost_usd).toBe(1.23);
  });

  it('validates null actual cost for submit-data rows', () => {
    const row = validateContributionRow({
      success_under_budget: false,
      actual_cost_usd: null,
    });
    expect(row.actual_cost_usd).toBe(null);
  });

  it('validates a benchmark row', () => {
    const row = validateContributionRow({
      schema_version: TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION,
      task_descriptor: makeTaskDescriptor(),
      allowed_models: ['planner-a', 'coder-a', 'reviewer-a'],
      selected_models: {
        planner: 'planner-a',
        coder: 'coder-a',
        reviewer: 'reviewer-a',
      },
      budget_usd: 10,
      actual_cost_usd: 2.2,
      wall_clock_seconds: 120,
      success_under_budget: true,
      completion_result: 'success',
      scorer_ref: 'router-benchmark/v1',
      observed_at: '2026-05-30T12:00:00.000Z',
      task_id: 'redacted-task',
      harness: 'wavemill',
    });
    expect('schema_version' in row && row.schema_version).toBe(TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION);
  });

  it('validates null actual cost for benchmark rows', () => {
    const row = validateContributionRow({
      schema_version: TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION,
      task_descriptor: makeTaskDescriptor(),
      allowed_models: ['planner-a', 'coder-a', 'reviewer-a'],
      selected_models: {
        coder: 'coder-a',
        reviewer: 'reviewer-a',
      },
      actual_cost_usd: null,
      success_under_budget: false,
      completion_result: 'success',
      observed_at: '2026-05-30T12:00:00.000Z',
    });
    expect(row.actual_cost_usd).toBe(null);
  });

  it('validates a full v2 benchmark row', () => {
    const row = validateContributionRow(makeV2Row());
    expect('schema_version' in row && row.schema_version).toBe(TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION_V2);
    expect('available_models' in row && row.available_models.coder_models[1]).toBe('deepseek-coder-v2');
  });

  it('validates a minimal v2 benchmark row', () => {
    const row = validateContributionRow(makeV2Row({
      budget_usd: undefined,
      actual_cost_usd: null,
      wall_clock_seconds: undefined,
      selected_models: {
        coder: 'coder-a',
        reviewer: 'reviewer-a',
      },
      available_models: {
        planner_models: [],
        coder_models: ['coder-a'],
        reviewer_models: ['reviewer-a'],
      },
      outcome_labels: {
        budget_label: 'unknown',
        cost_label: 'unknown',
        time_label: 'unknown',
        success_label: 'success',
      },
      candidate_pool: {
        scenario_id: 'default',
        scenario_kind: 'fallback',
        pool_size: 1,
      },
      sparse_cell: {
        cell_id: 'default',
        descriptor_signature: 'default',
        observed_count: 0,
        is_sparse: false,
      },
    }));
    expect('schema_version' in row && row.schema_version).toBe(TECHNICAL_TASK_ROUTER_ROW_SCHEMA_VERSION_V2);
  });

  it('exports the canonical harness outcome row field list', () => {
    expect(HARNESS_OUTCOME_ROW_FIELDS).toEqual([
      'schema_version',
      'task_descriptor',
      'allowed_models',
      'selected_models',
      'budget_usd',
      'actual_cost_usd',
      'wall_clock_seconds',
      'completion_result',
      'success_under_budget',
      'inference_log_id',
      'harness',
      'task_id',
      'observed_at',
      'harness_metadata',
    ]);
    expect(HARNESS_OUTCOME_ROW_SCHEMA_VERSION).toBe('harness_outcome_row/v1');
  });

  it('rejects rows missing success_under_budget', () => {
    expectContributionError(
      () => validateContributionRow({ actual_cost_usd: 2 }),
      'schema_validation_failed',
    );
  });

  it('rejects nested forbidden keys', () => {
    expectContributionError(
      () => validateContributionRow({
        success_under_budget: true,
        inputs: {
          nested: {
            prompt: 'secret',
          },
        },
      }),
      'forbidden_field',
    );
  });

  it('rejects raw EvalRecord-style fields', () => {
    expectContributionError(
      () => validateContributionRow({
        success_under_budget: true,
        original_prompt: 'unsafe raw task text',
      }),
      'forbidden_field',
    );
  });

  it('rejects forbidden keys nested in v2 metadata', () => {
    expectContributionError(
      () => validateContributionRow(makeV2Row({
        candidate_pool: {
          scenario_id: 'a',
          scenario_kind: 'challenger',
          pool_size: 2,
          prompt: 'secret',
        },
      })),
      'forbidden_field',
    );
  });

  it('rejects v2 rows with missing required metadata', () => {
    const row = makeV2Row();
    delete row.sparse_cell;
    expectContributionError(
      () => validateContributionRow(row),
      'schema_validation_failed',
    );
  });

  it('rejects v2 rows with invalid outcome labels', () => {
    expectContributionError(
      () => validateContributionRow(makeV2Row({
        outcome_labels: {
          budget_label: 'under_budget',
          cost_label: 'tiny',
          time_label: 'fast',
          success_label: 'success',
        },
      })),
      'schema_validation_failed',
    );
  });

  it('rejects v2 rows with negative candidate pool size', () => {
    expectContributionError(
      () => validateContributionRow(makeV2Row({
        candidate_pool: {
          scenario_id: 'challenger',
          scenario_kind: 'challenger',
          pool_size: -1,
        },
      })),
      'schema_validation_failed',
    );
  });

  it('rejects v2 rows with non-ISO observed_at', () => {
    expectContributionError(
      () => validateContributionRow(makeV2Row({
        observed_at: 'not-a-date',
      })),
      'schema_validation_failed',
    );
  });
});

// ────────────────────────────────────────────────────────────────
// isSubmitDataContributionRow – outcome diagnostic fields (HOK-2262)
// ────────────────────────────────────────────────────────────────

describe('isSubmitDataContributionRow – outcome diagnostic optional fields (HOK-2262)', () => {
  const baseRow = { success_under_budget: true };

  it('accepts row without any outcome diagnostic fields', () => {
    expect(isSubmitDataContributionRow(baseRow)).toBe(true);
  });

  it('accepts row with valid outcome_diagnostic enum values', () => {
    for (const value of ['eligible', 'ineligible_missing_outcome', 'ineligible_failed_outcome', 'unknown']) {
      expect(isSubmitDataContributionRow({ ...baseRow, outcome_diagnostic: value })).toBe(true);
    }
  });

  it('rejects row with invalid outcome_diagnostic enum value', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_diagnostic: 'bad_value' })).toBe(false);
  });

  it('accepts row with valid outcome_source enum values', () => {
    for (const value of ['feature_outcome_artifact', 'reconstructed', 'unknown']) {
      expect(isSubmitDataContributionRow({ ...baseRow, outcome_source: value })).toBe(true);
    }
  });

  it('rejects row with invalid outcome_source enum value', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_source: 'not_a_source' })).toBe(false);
  });

  it('accepts row with boolean outcome_artifact_present', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_artifact_present: true })).toBe(true);
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_artifact_present: false })).toBe(true);
  });

  it('rejects non-boolean outcome_artifact_present', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_artifact_present: 'yes' })).toBe(false);
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_artifact_present: 1 })).toBe(false);
  });

  it('accepts row with string[] outcome_missing_fields', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_missing_fields: ['merged', 'ciPassed'] })).toBe(true);
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_missing_fields: [] })).toBe(true);
  });

  it('rejects outcome_missing_fields when not an array of strings', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_missing_fields: 'merged' })).toBe(false);
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_missing_fields: [1, 2] })).toBe(false);
  });

  it('accepts row with valid outcome_failure_reason string', () => {
    expect(
      isSubmitDataContributionRow({ ...baseRow, outcome_failure_reason: 'incomplete_outcome' }),
    ).toBe(true);
  });

  it('rejects outcome_failure_reason when non-string', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, outcome_failure_reason: 42 })).toBe(false);
  });

  it('rejects row with unknown extra key', () => {
    expect(isSubmitDataContributionRow({ ...baseRow, unknown_key: 'oops' })).toBe(false);
  });
});
