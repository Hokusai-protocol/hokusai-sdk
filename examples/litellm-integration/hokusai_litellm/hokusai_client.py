...
from .errors import ContributionSubmissionFailed, RoutingCallFailed
...
class HokusaiHttpClient:
    ...
    def _build_route_request(
        ...
        request: dict[str, Any] = {
            "inputs": {
                "task": {
                    "description": "metadata-only LiteLLM route",
                    "task_type": metadata.get("task_type", "unknown"),
                },
                "routing": routing,
                "metadata": {
                    "external_task_id": request_id or "litellm",
    ...
    @staticmethod
    def _parse_recommendation(payload: Any) -> Recommendation:
        ...
        model = strategy.get("coder_model") or strategy.get("model") or metadata.get("coder_model")
        ...
        if inference_log_id is not None and not isinstance(inference_log_id, str):
            inference_log_id = None

        if model is None and inference_log_id is None and not predictions:
            raise RoutingCallFailed(
                "Hokusai routing response did not include a recommendation payload"
            )
