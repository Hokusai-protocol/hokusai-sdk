...
- Routing requires `HOKUSAI_API_KEY`. `HOKUSAI_OUTCOME_OPT_IN` controls outcome submission, and normal adapter precedence is `env > stored > default`.
...
- `resolveConsent()` defaults routing to enabled and outcome opt-in to `false`.
- `canRouteWithAuth()` requires an API key.
- `canSubmitOutcomeWithAuth()` requires an API key and separate outcome opt-in.
- Supported user-facing env vars are `HOKUSAI_API_KEY`, `HOKUSAI_OUTCOME_OPT_IN`, and `HOKUSAI_MODEL_ALLOWLIST`.
